...
def print_report(response_data, findings, verbose=False):
    ...
    if not response_data["success"]:
        console.print(
            f"[bold red]Error connecting to {response_data['url']}: {response_data.get('error')}[/bold red]"
        )
        return
    ...
    if findings:
        table = Table(
            title="Analysis Findings", show_header=True, header_style="bold magenta"
        )
        ...
        sorted_findings = sorted(
            findings, key=lambda x: severity_map.get(x["severity"], 3)
        )
